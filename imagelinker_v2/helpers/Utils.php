...
class Utils
{
    ...
    public function prepareRecordUpdatesData($inputFilePath, $action, $log){
        $imagesToLink = array();
        $lines = file($inputFilePath);
        if(empty($lines)){
            $log->logError($inputFilePath . " file is empty.");
        ...
        /* Gather information to link. */
        // UPDATE input file needs only object_id
        elseif(strtoupper($action) === "UPDATE"){
            foreach($lines as $line){
                $record = explode("\t", trim($line));
                if(sizeof($record) < 1){
...
//    }
    ...
    public function isCollectionFindResponseValid($response){
        if(!empty($response) && !empty($response->results) && sizeof($response->results) > 0){
